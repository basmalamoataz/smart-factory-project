...
void setup() {
  ...
  espClient.setInsecure(); // skip certificate check (fine for a class project)
...
void loop() {
  ...
  if (now - lastPublish >= publishInterval) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (i == 1 && demoTick > 5) { // machine2 = index 1
        if (temperature[i] < 95) temperature[i] += 0.8;
        if (vibration[i] < 12) vibration[i] += 0.15;
      }

      temperature[i] = clampf(temperature[i], 20, i == 1 ? 100 : 60);
      vibration[i]   = clampf(vibration[i], 0.5, i == 1 ? 15 : 5);
